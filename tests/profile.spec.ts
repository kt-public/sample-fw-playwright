...
test.describe("Profile Tests", { tag: "@profile" }, () => {
  ...
  test.afterEach(async ({ loginApi, profileApi }) => {
    await allure.step("Execute Api to reset profile name as before testing", async () => {
  ...
  test(
    ...
    async ({ productPage,profilePage }) => {
      ...
      await allure.step("Verify if new name is displayed in header correctly", async () => {
        await expect(await productPage.isCorrectProfileNameOnHeader(newName)).toBe(true);
      });
    },
  );
});
